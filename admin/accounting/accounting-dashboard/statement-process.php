...
if(isset($_POST['editfees'])){
  $id = $_POST['id'];
  $query = "SELECT DISTINCT statement_of_account.student_number, student_info.lastname, student_info.firstname, student_info.middlename, student_info.course_id, course.course_abbreviation, statement_of_account.semester, statement_of_account.schoolyear, statement_of_account.tuition FROM statement_of_account 
    INNER JOIN student_info ON statement_of_account.student_number = student_info.student_number INNER JOIN course ON student_info.course_id = course.course_id WHERE sao_id = $id";
