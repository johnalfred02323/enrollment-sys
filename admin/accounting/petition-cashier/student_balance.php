...
$query = mysqli_query($conn, "SELECT * FROM transaction_fees WHERE semester='$sem' AND schoolyr = '$sy' AND student_number = '$query'") or die(mysqli_error($conn));
 ...
 if(isset($_POST['addpayment'])){
  ...
  $stmt = $pdo_conn->prepare("INSERT INTO transaction_fees (student_number, official_receipt, cash_rendered, balance, amount, status, cashier, schoolyr, semester, `date`) VALUES (?,?,?,?,?,?,?,?,?,CURRENT_DATE())");
  $stmt->bindParam(1, $sn1);
  $stmt->bindParam(2, $or);
  $stmt->bindParam(3, $cash);
  $stmt->bindParam(4, $bal);
  $stmt->bindParam(5, $am);
  $stmt->bindParam(6, $stat);
  $stmt->bindParam(7, $cashier);
  $stmt->bindParam(8, $sy);
  $stmt->bindParam(9, $sem);
