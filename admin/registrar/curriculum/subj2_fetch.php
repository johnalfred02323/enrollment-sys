...
$columns = array(
  ...
  array('db' => 'Semester', 'dt' => 7),
	...
	array('db' => 'updated_at', 'dt' => 9, 'formatter' => function($d,$row){return date('jS M Y', strtotime($d));}),
		array('db' => 'subj_id', 'dt' => 10, 'formatter' => function($d,$row){return '<button id="'.$d.'" class="edit-user pull-xs-right edit2" data-toggle="modal" data-target="#subject_modal" ripple><i class="fas fa-edit"></i> EDIT</button> <button id="'.$d.'" class="delete-user pull-xs-right delete2" data-toggle="modal" data-target="#yesorno" ripple><i class="far fa-trash-alt"></i> DELETE</button>';})
...
$where = "(curriculum_title = '".$title."' OR curriculum_title = 'All') AND Semester = 'Second Semester' AND Year = '".$year."'";
