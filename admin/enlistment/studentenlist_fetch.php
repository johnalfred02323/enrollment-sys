...
$table = <<<EOT
 (
    ...
    LEFT JOIN schedule ON student_enrollment_record.sched_id = schedule.sched_id  GROUP BY student_number
 ) temp
EOT;
...
$columns = array(
	...
	array('db' => 'date', 'dt' => 5, 'formatter' => function($d,$row){return date('jS M Y', strtotime($d));}),
		array('db' => 'student_number', 'dt' => 6, 'formatter' => function($d,$row){return '<button id="'.$d.'" class="view-user pull-xs-right viewdata" ripple><i class="fas fa-eye"></i> VIEW</button> <button id="'.$d.'" class="edit-user pull-xs-right editdata" ripple><i class="fas fa-edit"></i> EDIT</button>';})
...
if(isset($_GET['course']) && isset($_GET['major']) && isset($_GET['sy']) && isset($_GET['sem']) )
{
    ...
    $sem = $_GET['sem']; 
    $major = $_GET['major']; 
    $where = "school_year = '$sy' AND semester = '$sem' AND course = '$course' AND major = '$major' AND status='Enlisted'";
}
