...
if(isset($_POST['viewsched'])){
    ...
    if(mysqli_num_rows($result) > 0)  
    {
      while($rows=mysqli_fetch_array($result))
      {
        ...
        $query2 = "SELECT COUNT(student_enrollment_record.student_number) AS num FROM student_enrollment_record INNER JOIN schedule ON student_enrollment_record.sched_id = schedule.sched_id INNER JOIN subject ON subject.subj_id = schedule.subj_id WHERE subject.subject_code = '$subjcode' && schedule.section = '$section' && schedule.school_year = '$schoolyear' && schedule.semester = '$sem'"; 
