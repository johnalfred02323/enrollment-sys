...
require('../../../src/fpdf/fpdf.php');
    ...
    $partial = $_GET['scholar'];  
    $fees = $_GET['miscellaneous'];
    $totalunits = $_GET['tunit1'];
    $perunits = $_GET['perunit'];
    $amount = $_GET['amount'];
    $tunits1 = $_GET['tunit2'];
    $disc = $_GET['discount'];
    ...
    $query = "SELECT student_info.student_number, student_info.lastname, student_info.firstname, student_info.middlename, student_info.suffix,
      student_info.course_id, course.course_abbreviation, course.course_major as major, student_enrollment_record.date, schedule.semester, schedule.school_year
      ...
      Inner join schedule on schedule.sched_id = student_enrollment_record.sched_id 
        INNER JOIN course ON student_info.course_id = course.course_id WHERE student_info.student_number = '$studno'";
    ...
    $pdf->Cell(1);
    $pdf->SetFont('Arial', '', 8);
    $pdf->Cell(43, 6, ''.$_GET['scholar'].'', 1, 1,'C');
    ...
    $pdf->Cell(1);
    $pdf->SetFont('Arial', '', 8);
    $pdf->Cell(27, 6, ''.$row['course_abbreviation'].'', 1, 0,'C');
    $pdf->SetFont('Arial', 'B', 8);
    $pdf->Cell(1);
    $pdf->Cell(27, 6, 'Status', 1, 0,'C');
    $pdf->Cell(1);
    $pdf->SetFont('Arial', '', 8);
    $pdf->Cell(29, 6, ''.$_GET['status'].'', 1, 1,'C');
    ...
    $pdf->LN(26);
    $pdf->Cell(141);
    $pdf->Cell(20, 6, ''.$_GET['tunit1'].'', 0, 0, 'C');
    $pdf->Cell(-15);
    $pdf->Cell(20, 6, 'x', 0, 0, 'C');
    $pdf->Cell(-15);
    $pdf->Cell(20, 6, ''.$_GET['perunit'].'', 0, 0, 'C');
    $pdf->Cell(-1);
    $pdf->Cell(20, 6, ''.$_GET['tunit2'].'', 0, 1, 'C');
    $pdf->LN(5);
    $pdf->Cell(143);
    $pdf->Cell(20, 6, ''.$_GET['miscellaneous'].'', 0, 0 , 'C');
    $pdf->Cell(7);
    $pdf->Cell(20, 6, ''.$_GET['miscellaneous'].'', 0, 1, 'C');
    ...
    $pdf->Cell(11, 10, ''.$_GET['tunit1'].'', 0, 0, 'C');
