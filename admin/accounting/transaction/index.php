...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    <div class="breadcrumb-item active" aria-current="page"><a href="../cashier-dashboard">Dashboard</a> / Transaction / Other Transaction</div>
  </ol>
</nav>
    ...
    <label id="sy" hidden=""><?php echo $row['school_year']; ?></label>

    <label id="semester" hidden=""><?php echo $row['semester']; ?></label>
...
<div class="card shadow-sm mb-4"> <!-- card start -->
  ...
  <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
      ...
      <h6 class="m-0 font-weight-bold card-text-title-head">Other Transaction</h6>
  ...
  <br>
   ...
   <div id="transaction_div" class="row" style="display: none;"> <!-- ROW Start Here -->
       ...
       <input type="text" class="form-controls" id="change" aria-label="Default" placeholder="Change" aria-describedby="inputGroup-sizing-default" disabled=""><br>
        ...
        <div class="col-ms-3">
        <!-- Modal Add -->
        <button id="" class="view-user pull-xs-right edit" ripple data-toggle="modal" data-target="#scholar_modal" > <i class="fas fa-plus"></i> Add</button>
...
$(document).on('click', '#done', function(){
  ...
  {
    $( "div.failed" ).fadeIn( 300 ).delay( 3000 ).fadeOut( 400 );
    $('#failedmsg').html("Please Insert Item");
    $('#rendered').val('');
  }
  else{
    var i = 0;
  ...
  if (result < 0)
   ...
   $('#change').val(result);
    ...
    $.ajax({
      url:"transaction.php",
      method:"POST",
      data: {"add":1,allitems:allitems,sn:sn,or:or,rn:rn,ch:ch,tr:tr,uc:uc,ln:ln,fn:fn,mn:mn,sy:sy,sem:sem,num1:num1,dscpt:dscpt},
      success:function(data){
        if(data=='0')
        {
          $('#success').modal('show');
